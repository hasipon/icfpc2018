...
void simple(const int y, const Filled& filled) {
	vector<P> plan;
	bool bb = true;
	for (int z = size_z-1; z >= 0; -- z, bb = !bb) {
		if (bb) {
			for (int x = size_x-1; x >= 0; -- x) if (Fget(filled, P(x,y,z))) {
				plan.push_back(P(x,y+1,z));
			}
		} else {
			for (int x = 0; x < size_x; ++ x) if (Fget(filled, P(x,y,z))) {
				plan.push_back(P(x,y+1,z));
			}
		}
	}
...
void smart(const int y, const Filled& filled) {
	...
	for (int x = 0; x < size_x; ++ x) for (int z = 0; z < size_z; ++ z) if (Fget(filled, P(x,y,z))) {
		(Fget(filled, P(x,y-1,z)) ? gr : not_gr).insert(P(x,y+1,z));
	}
	vector<P> plan;
	while (!gr.empty()) {
		plan.insert(plan.end(), gr.begin(), gr.end());
