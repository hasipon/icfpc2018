			...
			for (auto p : bpos) {
				...
				for (int i = 0; i < 2; i++) {
					...
					if (a[base][i].count(p)) {
						...
						if(a[base][i][p] == 0 && x ==0 && y == 0)continue;
						P nd = P(1, 0, 0);
					...
					if (b[base][i].count(p) && y!=0){
						...
						if(b[base][i][p] == 0 && x ==0 && y == 0)continue;
						P nd = P(1, 0, 0);
					...
					if (c[base][i].count(p) && x!=0) {
						...
						if(c[base][i][p] == 0 && x ==0 && y == 0)continue;
						P nd = P(-1, 0, 0);
					...
					if (d[base][i].count(p) && x !=0 && y!=0) {
						...
						if(d[base][i][p] == 0 && x ==0 && y == 0)continue;
						P nd = P(-1, 0, 0);
...
void disassemble() {
	...
	vector<int> xs = {bb.x.first, min(30, bb.x.second-1)};
	vector<int> ys = {bb.y.first, min(30, bb.y.second-1)};
	vector<int> zs;
