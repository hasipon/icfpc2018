struct AsmMain : OutputBase {
	...
	const vector<P> Nears { P(0,-1,0), P(1,-1,0), P(-1,-1,0), P(0,-1,1), P(0,-1,-1) };
	set<P> CanMove;
...
void do_fill(P& pos, const set<P>& goal) {
	set<P> a;
	for (auto p : goal) a.insert(P(p.x, p.y-1, p.z));

	for (;;) {
		for (auto d : Nears) if (a.count(pos + d)) {
			Fill(d);
			a.erase(pos + d);
		}
		if (a.empty()) break;
		tuple<int,int,P> best(0, 0, P());
		for (auto m : CanMove) {
			int score = 0;
			for (auto d : Nears) if (a.count(pos + m + d)) ++ score;
			best = max(best, make_tuple(score, mdist(m), m));
		}
		if (get<0>(best) > 0) {
			moveTo(pos, pos + get<2>(best));
			pos += get<2>(best);
		} else {
			pair<int, P> nearest(1<<30, P());
			for (auto p : a) nearest = min(nearest, {mdist(pos - p), p});
			moveTo(pos, nearest.second + P(0,1,0));
			pos = nearest.second + P(0,1,0);
		}
	}
}
