...
void do_fill(P& pos, const set<P>& _goal) {
  set<P> goal = _goal;
  while (goal.size()) {
    P start = *goal.begin();
    P end = start;
    ...
    assert(end == curr);
    GFill(Point(0, -1, 0), end - start);
    GFill(Point(0, -1, 0), start - end);
