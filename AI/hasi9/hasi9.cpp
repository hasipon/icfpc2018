...
void move(P& pos, const P& to) {
	...
	if (d.x > 15) { SMove(P(15,0,0)); d.x -= 15; }
	if (d.x < -15) { SMove(P(-15,0,0)); d.x += 15; }
	if (d.y > 15) { SMove(P(0,15,0)); d.y -= 15; }
	if (d.y < -15) { SMove(P(0,-15,0)); d.y += 15; }
	if (d.z > 15) { SMove(P(0,0,15)); d.z -= 15; }
	if (d.z < -15) { SMove(P(0,0,-15)); d.z += 15; }
...
void clear() {
	...
	for (int y = 0; y < R-1; ++ y) {
		for (int z = 1; z < R-1; ++ z) {
			vector<int> xs;
			for (int x = 1; x < R-1; ++ x) if (Src(x,y,z)) xs.push_back(x);
			if (xs.empty()) continue;
			move(pos, P(xs[0], pos.y, pos.z));
			if (abs(y-pos.y) >= 2) {
				move(pos, P(pos.x, y > pos.y ? y-1 : y+1, pos.z));
			}
			if (abs(z-pos.z) >= 2) {
				move(pos, P(pos.x, pos.y, z > pos.z ? z-1 : z+1));
			}
			for (int x : xs) {
				move(pos, P(x, pos.y, pos.z));
				Void(P(x,y,z) - pos);
			}
		}
	}
	move(pos, P(pos.x,pos.y,0));
	move(pos, P(pos.x,0,0));
	move(pos, P(0,0,0));
}
